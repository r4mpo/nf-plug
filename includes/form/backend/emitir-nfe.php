...
try {
  ...
  global $wpdb;
  $wpdb->query($wpdb->prepare("UPDATE wp_wc_order_stats SET chave_nota_fiscal_plug='teste' WHERE order_id = $_POST[numero_do_pedido_woocommerce]"));
  $wpdb->query($wpdb->prepare("UPDATE wp_wc_order_stats SET situacao_nota_fiscal_plug='teste' WHERE order_id = $_POST[numero_do_pedido_woocommerce]"));
} catch (\Throwable $th) {
  echo $th->getMessage();
}
?>